...
We selected [Drogon](https://github.com/drogonframework/drogon) because of its mature coroutine ecosystem -- one of the few C++ libraries to provide first-class support for asynchronous I/O via C++20 coroutines. Our choice was driven by several project-specific needs:

- **API Abstraction vs. Raw Networking**: While libraries such as `boost::asio` are excellent for general-purpose networking, Drogon's MVC structure allowed us to design a clean REST API using declarative routing and stateful connections, shielding our business logic from HTTP/1.1 protocol complexities.
- **Integrated Database Utilities**: We leveraged Drogon's coroutine-aware Object Relational Mappers (ORM) `CoroMapper` to persist calculated schedules. This integration allowed us to insert the thousands of data-points generated by every workload into a PostgreSQL database asynchronously, ensuring the persistence layer remains as non-blocking as the optimization engine itself.
